...
RSpec.describe "Questions", type: :request do
  ...
  describe "質問の一覧表示" do
    ...
    context "質問が3件登録されている状態で全質問一覧ページに遷移すると" do
      it "3件の質問が表示される" do
        question
        create(:question, user_id: user.id, book_id: book.id)
        create(:question, user_id: user.id, book_id: book.id)
    ...
    context "質問が登録されていない状態で全質問一覧ページに遷移すると" do
      it "空の配列が返ってくる" do
        get all_questions_questions_path
        expect(response).to have_http_status(200)
        expect(JSON.parse(response.body).length).to eq(0)
  ...
  describe "質問の検索機能" do
    ...
    context "2件の質問が登録されている状態で検索キーワードが2件の質問に一致すると" do
      it "2件の質問が表示される" do
        get search_questions_questions_path, params: {
