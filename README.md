...
しかし、Vuexはページリロード時に状態がリセットされてしまうため、ログイン状態の維持が困難でした。<br>

そこで、Nuxt.jsのプラグイン機能を用いて、この問題に対処しました。具体的には「ページリロード時にFirebaseでログイン状態を再確認し、その結果をもとにデータベースからユーザー情報を取得し、ログイン状態とユーザー情報をVuexに保存する」という処理をプラグイン内で実装しました。<br>

このようなアプローチによって、ページリロード後もログイン状態を維持することができるようになりました。
