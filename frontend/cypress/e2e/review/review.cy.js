...
it("レビュー詳細ページにアクセスすると、レビューの詳細情報が表示される", () => {
  ...
  it.only("レビュー詳細ページにアクセスすると、レビューに関連する参考書情報が表示される", () => {
    cy.visit("/")
...
describe("レビュー投稿", () => {
  ...
  it("ログイン状態で不正なパラメーターでレビューを投稿すると、レビュー投稿に失敗する", () => {
    ...
    cy.get("[data-cy=review-title-form]").click()

    cy.get("[data-cy=review-title-form]").blur()

    cy.contains("タイトルを入力してください")

    cy.get("[data-cy=review-content-form]").click()
...
describe("レビュー編集", () => {
  ...
  it("ログイン状態で自分のレビューを不正なパラメーターで編集すると、レビューの編集に失敗する", () => {
    ...
    cy.contains("サンプルレビュータイトル3").click()
...
describe("レビュー削除", () => {
  ...
  it("未ログイン状態だと、レビューの削除ボタンが表示されない", () => {
    ...
    cy.contains("サンプルレビュータイトル4").click()
