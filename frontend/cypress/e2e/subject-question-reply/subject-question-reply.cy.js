...
describe("科目別返信一覧", () => {
  ...
  it("科目別返信が2件投稿されている科目別質問の詳細にアクセスすると、2件の科目別返信が表示される", () => {
    ...
    cy.contains("サンプル科目別質問タイトル1").scrollIntoView().click()
...
describe("科目別返信編集", () => {
  ...
  it("ログイン状態で自分の科目別返信を編集すると、科目別返信の編集に成功する", () => {
    ...
    cy.get("[data-cy=each-subject-question-subject-question-replies]").eq(0).find("textarea").should("have.value", "編集用科目別返信").click()
  ...
  it("ログイン状態で自分の科目別返信を不正なパラメーターで編集すると、科目別返信の編集に失敗する", () => {
    ...
    cy.get("[data-cy=each-subject-question-subject-question-replies]").eq(0).find("textarea").should("have.value", "編集失敗用科目別返信").click()
  ...
  it("ログイン状態で他人の科目別返信詳細ページにアクセスすると,科目別返信編集ボタンが表示されない", () => {
    ...
    cy.contains("他ユーザー投稿科目別質問タイトル").click()

    cy.contains("質問詳細(科目別)")

    cy.get("[data-cy=each-subject-question-subject-question-replies]").eq(0).find("textarea").should("have.value", "他ユーザー投稿科目別返信").click()
...
describe("科目別返信削除", () => {
  ...
  it("ログイン状態で自分の科目別返信を削除すると、科目別返信の削除に成功する", () => {
    ...
    cy.get("[data-cy=each-subject-question-subject-question-replies]").eq(0).find("textarea").should("have.value", "削除用科目別返信").click()
  ...
  it("未ログイン状態だと、科目別返信の削除ボタンが表示されない", () => {
    ...
    cy.contains("削除失敗用科目別質問").click()

    cy.contains("質問詳細(科目別)")

    cy.get("[data-cy=each-subject-question-subject-question-replies]").eq(0).find("textarea").should("have.value", "削除失敗用科目別返信").click()
