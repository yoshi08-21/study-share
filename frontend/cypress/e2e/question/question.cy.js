...
describe("質問の検索機能", () => {
  ...
  context("質問タイトルで検索", () => {
    ...
    it("質問一覧ページで質問を質問タイトルで検索すると、キーワードが完全一致する質問が表示される", () => {
      ...
      cy.get("[data-cy=search-questions-button]").click({ force: true })
...
describe("質問の並べ替え機能", () => {
  ...
  it("質問を「新着順」で並び替えると、新着順に質問が表示される", () => {
    ...
    cy.get("[data-cy=sort-questions-select]").click({ force: true })
  ...
  it("質問を「返信が多い順」で並び替えると、返信が多い順に質問が表示される", () => {
    ...
    cy.contains("返信が多い順").click({ force: true })
...
describe("質問の絞り込み機能", () => {
  ...
  it("質問を「英文法」の科目で絞り込むと、科目が英文法の質問のみが表示される", () => {
    ...
    cy.get("[data-cy=filter-questions-select]").click({ force: true })

    cy.get(".v-menu__content").contains("英文法").click({ force: true })
...
describe("コンテンツナビゲーター", () => {
  ...
  it("質問詳細ページで「次の質問」「前の質問」のナビゲーション機能が動作する", () => {
    cy.visit("/")
...
describe("質問投稿", () => {
  ...
  it("ログイン状態で不正なパラメーターで質問を投稿すると、質問投稿に失敗する", () => {
    ...
    cy.get("[data-cy=question-title-form]").click({ force: true })

    cy.get("[data-cy=question-title-form]").blur()

    cy.contains("タイトルを入力してください")

    cy.get("[data-cy=question-content-form]").click({ force: true })
...
describe("質問編集", () => {
  ...
  it("ログイン状態で自分の質問を不正なパラメーターで編集すると、質問の編集に失敗する", () => {
    ...
    cy.contains("サンプル質問タイトル3").click({ force: true })
...
describe("質問削除", () => {
  ...
  it("未ログイン状態だと、質問の削除ボタンが表示されない", () => {
    ...
    cy.contains("サンプル参考書3").scrollIntoView().click({ force: true })

    cy.get(".v-tabs").contains("質問").click({ force: true })

    cy.contains("サンプル質問タイトル4").click({ force: true })
