...
describe("返信詳細", () => {
  ...
  it("返信詳細ページにアクセスすると、返信の詳細情報が表示される", () => {
    ...
    cy.get("[data-cy=each-question-replies]").eq(0).find("textarea").should("have.value", "英語の返信1").click({ force: true })
...
describe("コンテンツナビゲーター", () => {
  ...
  it("返信詳細ページで「次の返信」「前の返信」のナビゲーション機能が動作する", () => {
    cy.visit("/")
    ...
    cy.get("[data-cy=each-question-replies]").first().scrollIntoView().click()
...
describe("返信編集", () => {
  ...
  it("ログイン状態で自分の返信を編集すると、返信の編集に成功する", () => {
    ...
    cy.get("[data-cy=each-replies]").eq(0).scrollIntoView().click()
  ...
  it("ログイン状態で他人の返信詳細ページにアクセスすると,返信編集ボタンが表示されない", () => {
    ...
    cy.get("[data-cy=each-question-replies]").eq(1).find("textarea").should("have.value", "サンプル返信本文2").click({ force: true })
...
describe("返信削除", () => {
  ...
  it("ログイン状態で自分の返信を削除すると、返信の削除に成功する", () => {
    ...
    cy.get("[data-cy=each-replies]").eq(1).find("textarea").should("have.value", "英語の返信1").click({ force: true })
  ...
  it("未ログイン状態だと、返信の削除ボタンが表示されない", () => {
    ...
    cy.get("[data-cy=each-question-replies]").last().find("textarea").should("have.value", "英語の返信3").click({ force: true })
